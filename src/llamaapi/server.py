...
from llamaapi.exceptions import ResourceNotFoundError, ServerError, ValidationError
...
class API:
    ...
    def register_error_handler(
        self, exception_type: Type[Exception], handler: Callable[[Exception], Response]
    ) -> None:
        """
        ...
        async def middleware(request: Request) -> Request:
            ...
            if data is not None:
                try:
                    ...
                    raise ValidationError(
                        f"JSON validation failed: {str(e)}", errors={"schema": str(e)}
                    )
            return request
